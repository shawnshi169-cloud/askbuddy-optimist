...
| Canonical Topic | EC-3B2B backend 已部署验证；[B2C HTTP consumer gate](./ec3b2c-canonical-topic-consumer-gate.md) 已验证 | shared Question/Experience/Matching/Discover/Community semantic layer | 无 seed/backfill，不复用 hashtag ID；contract 0..N，当前 UI 仍发送 []，无 Topic picker / Experience hookup | EC-3 | A | YES，限已验证的三个 RPC contract，不代表 Discovery/UI 完成 |
