...
`topic:manifest:validate`: **PASS**. The strict validator rejects unknown fields, invalid identity/status, empty terms, wrong ordering, duplicates and all normalized canonical/alias collisions. It does not rewrite inputs. `validateCoreV1Manifest` additionally requires `manifestId=core-v1`, exactly 79 active entries, no deferred 人像摄影, and a fixed approved-content digest binding every canonical name, alias, UUID and array order. The tests independently freeze the approved name/alias fixture and exact manifest bytes, including every UUID.

`validateManifestShape` is generic for schemaVersion 1. Its manifest identifier is a non-empty lowercase ASCII kebab identifier (for example `core-v2`), not a hardcoded `core-v1` value. It imposes no 79-entry limit, active-only restriction or curated-content allowlist. `planManifest` can compare such structurally valid manifests without authorizing any apply. Tests use existing identities/fixtures only; no future manifest or Topic ID is created. The current command and `previewCoreV1` remain intentionally Core-v1-specific and retain the initial empty-Production gate and unchanged planHash.

Core Seed v1 is a frozen initial curated manifest, not a closed Canonical Topic namespace. After approval its existing identities/content remain immutable; **79 is not a ceiling on global Production Topic count**. A future approved Topic such as 澳洲留学 would receive a new cryptographically secure UUID v4 exactly once, not reuse a Core UUID or derive its identity from its name. Adding that governed fact would not require a Topic schema migration or alter any existing identity. Such content requires a separately governed future manifest/workflow; that workflow, core-v2 content and 澳洲留学 itself are NOT implemented or added here. Production-only Topics absent from core-v1 remain untouched. Structural extensibility is not content approval, a new apply path, Candidate runtime or UI authorization.
